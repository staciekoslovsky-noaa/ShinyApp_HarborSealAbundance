...
survey_polygons <- survey_polygons[!is.na(survey_polygons@data$abund_est), ]


# ----------------------------------------------------------------------------------------------------------------------------
# NOTE TO SKH: Recreate this section based on either function from JML or by applying this code to the data hosted on Git
...
for(j in 1:length(survey_polygons@polygons)){
  for(k in 1:length(survey_polygons@polygons[[j]]@Polygons)){
    # Move the polygons (besides the Aleutians)
    for (i in 1:length(survey_polygons@polygons[[j]]@Polygons[[k]]@coords[,1])){
      if (survey_polygons@polygons[[j]]@Polygons[[k]]@coords[i, 1] < 0){
        survey_polygons@polygons[[j]]@Polygons[[k]]@coords[i, 1] <- survey_polygons@polygons[[j]]@Polygons[[k]]@coords[i, 1] + 360
    ...
    if(survey_polygons@polygons[[j]]@labpt[1] < 0){
      # Move centroids 360 degrees again
      x_vals <- append(x_vals, survey_polygons@polygons[[j]]@labpt[1] + 360)
      # Move centroids across the map as well
      survey_polygons@polygons[[j]]@labpt[1] <- survey_polygons@polygons[[j]]@labpt[1] + 360
    }
    else{
      x_vals <- append(x_vals, survey_polygons@polygons[[j]]@labpt[1])
    }
  
    y_vals <- append(y_vals, survey_polygons@polygons[[j]]@labpt[2])
}
...
ui <- fluidPage(theme = shinytheme("superhero"),
                ...
                fluidRow(
                  column(3, tabsetPanel(
                    ...
                    tabPanel("Contact", HTML(contact_info)),
    ...
    output$map1 <- renderLeaflet({
      ...
      map %>% 
        ...
        addPolygons(data = survey_polygons,
                    ...
                    #label = survey_polygons@data$stockname, 
                    weight = 1,
        ...
        if(is.null(input$map1_draw_new_feature) || (input$map1_draw_new_feature$properties$feature_type == "circle")){
          ...
          for(j in 1:length(survey_polygons@polygons)){
              ...
              if(is_inside_polygon(feature_x, feature_y, centroid_x, centroid_y)){
                ...
                overlapping_ids <- append(overlapping_ids, survey_polygons@data$id.x[j])
                }
              }
          
          #Subset the data accordingly
          overlapping_data <- subset(survey_polygons@data, survey_polygons@data$id.x %in% overlapping_ids)
